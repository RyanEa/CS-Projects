...
public class RedBlackTreeMap<TKey extends Comparable<TKey>, TValue> {
	...
	private class Node {
		...
		public String toString() {
			return "(" + mKey + ", " + mValue + ")" + mLeft.mKey + "---" + mRight.mKey +"@@@" +mParent;
		}
	}
   ...
   private void checkBalance(Node n) {
      ...
      else if (!n.mParent.mIsRed){return;}
	else if(getUncle(n) != null && n.mParent.mIsRed && getUncle(n).mIsRed) {
          n.mParent.mIsRed =false;
          getUncle(n).mIsRed = false;
          getGrandparent(n).mIsRed=true;
          checkBalance(getGrandparent(n));
      }
	else if (getGrandparent(n) != null && n.mParent.mRight == n && getGrandparent(n).mLeft == n.mParent) {
        rotateLeft(n.mParent);
      }
    else if(getGrandparent(n) != null && n.mParent.mLeft == n && getGrandparent(n).mRight == n.mParent){
       rotateRight(n.mParent);
    }
    if ( getGrandparent(n) != null && n.mParent != null &&
            n.mParent.mLeft == n && n.mParent == getGrandparent(n).mLeft){
        rotateLeft(n.mParent);
        getGrandparent(n).mIsRed= !getGrandparent(n).mIsRed;
        n.mParent.mIsRed= !n.mParent.mIsRed;
    }
       else if(getGrandparent(n) != null && n.mParent != null &&
            n.mParent.mRight == n && n.mParent == getGrandparent(n).mRight) {
        rotateRight(n.mParent);
        getGrandparent(n).mIsRed= !getGrandparent(n).mIsRed;
        n.mParent.mIsRed= !n.mParent.mIsRed;
    }
   ...
   // and color of each node.
	...
	private Node getUncle(Node n) {
        ...
        if(getGrandparent(n) == null || getGrandparent(n.mParent) == null ||
                getGrandparent(n.mParent).mRight == null || getGrandparent(n.mParent).mLeft == null){
		return null;}//Checks all paths up the tree and see if any are null
        if(getGrandparent(n) == getGrandparent(n.mParent).mRight)
            return n.mParent.mParent.mParent.mLeft;//if grenadparent == grandparent go get uncle
        return n.mParent.mParent.mParent.mRight;
	}
