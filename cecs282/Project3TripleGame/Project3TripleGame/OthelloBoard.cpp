...
OthelloBoard::OthelloBoard() :mBoard() {
...
void OthelloBoard::ApplyMove(GameMove *move) {
   OthelloMove *p = dynamic_cast<OthelloMove*>(move);
   int row = p->mRow, col = p->mCol;
   if (row != -1 || col != -1){
      mPassCount = 0;
      mBoard[p->mRow][p->mCol] = (mNextPlayer == 'B') ? 1 : -1;
      ...
      for (int y = 1; y >= -1; y--) {
         for (int x = -1; x <= 1; x++) {
            ...
            p->AddFlipSet(OthelloMove::FlipSet(count, y, x));
            while (count-- > 0 && --n > 0){
